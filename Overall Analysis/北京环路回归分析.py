...
output_dir = r"C:\Users\sq\Desktop\11.27\12.2结果2（ln设施点+1）"
...
# 检查必要的列是否存在
# 注意：根据用户之前的需求，应该有i_count和j_count列
required_cols = ['i_count', 'j_count']
...
if missing_cols:
    ...
    for required in required_cols:
        if required not in all_cols:
            # 尝试查找包含关键字的列
            if required == 'i_count':
                similar = [col for col in all_cols if ('i' in col.lower() and 'count' in col.lower()) or '出发' in col]
            elif required == 'j_count':
                similar = [col for col in all_cols if ('j' in col.lower() and 'count' in col.lower()) or '到达' in col]
            else:
...
# 检查是否还有其他回归分析需要的列
# 注意：原代码需要以下列：OD_ij, POP_j, BORDER, DIS_ij
# 但根据您的需求，我们可能需要调整或确认这些列是否存在
regression_cols_needed = ['OD_ij', 'POP_j', 'BORDER', 'DIS_ij']
available_regression_cols = [col for col in regression_cols_needed if col in df.columns]
missing_regression_cols = [col for col in regression_cols_needed if col not in df.columns]

if missing_regression_cols:
    print(f"警告: 缺少回归分析所需列: {missing_regression_cols}")
    print(f"可用的回归分析列: {available_regression_cols}")
    
    # 如果缺少关键列，我们可以根据现有列调整模型
    if 'OD_ij' not in df.columns:
        print("错误: 缺少因变量 OD_ij，无法进行回归分析")
        exit()
...
# 检查是否需要处理其他自变量
if 'POP_j' in df.columns:
    df['ln_POP_j'] = np.log(df['POP_j'])

# 删除缺失值
# 根据可用的列创建清理列表
clean_cols = ['ln_OD', 'ln_i_count', 'ln_j_count_plus1']
if 'ln_POP_j' in df.columns:
    clean_cols.append('ln_POP_j')
if 'BORDER' in df.columns:
    clean_cols.append('BORDER')
if 'DIS_ij' in df.columns:
    clean_cols.append('DIS_ij')
...
# 描述性统计
print("\n描述性统计:")
desc_stats_cols = ['ln_OD', 'ln_i_count', 'ln_j_count_plus1']
if 'ln_POP_j' in df_clean.columns:
    desc_stats_cols.append('ln_POP_j')
if 'BORDER' in df_clean.columns:
    desc_stats_cols.append('BORDER')
if 'DIS_ij' in df_clean.columns:
    desc_stats_cols.append('DIS_ij')

desc_stats = df_clean[desc_stats_cols].describe()
print(desc_stats)
...
# 相关性分析
print("\n相关性矩阵:")
corr_matrix = df_clean[desc_stats_cols].corr()
print(corr_matrix)
...
# 准备回归数据
# 根据可用变量构建X
X_columns = ['ln_i_count', 'ln_j_count_plus1']
if 'ln_POP_j' in df_clean.columns:
    X_columns.append('ln_POP_j')
if 'BORDER' in df_clean.columns:
    X_columns.append('BORDER')
if 'DIS_ij' in df_clean.columns:
    X_columns.append('DIS_ij')
...
# 4. 每个自变量的残差图
num_vars = min(len(X_columns), 3)  # 最多显示3个自变量的残差图
...
with open(os.path.join(output_dir, '回归方程.txt'), 'w', encoding='utf-8') as f:
    ...
    if 'ln_POP_j' in X_columns:
        f.write("ln_POP_j = ln(POP_j)\n")
    if 'BORDER' in X_columns:
        f.write("BORDER = BORDER (0/1变量)\n")
    if 'DIS_ij' in X_columns:
...
# 绘制变量分布图
# 选择要绘制的变量
variables_to_plot = ['ln_i_count', 'ln_j_count_plus1', 'ln_OD']
if 'ln_POP_j' in df_clean.columns:
    variables_to_plot.append('ln_POP_j')
if 'BORDER' in df_clean.columns:
    variables_to_plot.append('BORDER')
if 'DIS_ij' in df_clean.columns:
    variables_to_plot.append('DIS_ij')
